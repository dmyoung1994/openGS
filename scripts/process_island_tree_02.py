"""Build a whole-component mesh LOD from the exact Poly Haven Island Tree 02 GLB."""
...
SOURCE_SHA256 = "6281dace4ca5a75c8799276724a8e4d01bfb969942977018ceb1620801b06b91"
PIPELINE_VERSION = "island-tree-02-blender-5.2-whole-component-lod@2"
TARGETS = {"island_tree_02": 6000, "island_tree_02_leaves": 250000,
           "island_tree_02_branches": 75000}
SOURCE_MATERIAL_ORDER = ["island_tree_02", "island_tree_02_leaves", "island_tree_02_branches"]
...
def main() -> None:
    ...
    if hashlib.sha256(source.read_bytes()).hexdigest() != SOURCE_SHA256:
        raise RuntimeError("Island Tree 02 source GLB SHA-256 mismatch")
    ...
    for part in parts:
        ...
        target = TARGETS[material_name]
        before = len(part.data.polygons)
        if material_name == "island_tree_02":
            modifier = part.modifiers.new("island_tree_02_lod1_decimate", "DECIMATE")
    ...
    # Match the source GLB's one-node, three-primitive material contract. GLTFLoader
    # exposes those primitives in source material order; exporting three sibling
    # objects lets Blender traversal order pair branches with leaves at runtime.
    # Rejoin the processed source components and explicitly remap each polygon to
    # the original trunk, leaves, branches material order before export.
    bpy.ops.object.select_all(action="DESELECT")
    for part in parts:
        part.select_set(True)
    joined = next(part for part in parts if part.name == "island_tree_02")
    ...
    joined.name = "island_tree_02_LOD1"
    bpy.ops.object.select_all(action="DESELECT")
