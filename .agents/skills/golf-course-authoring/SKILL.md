...
0. **Read `references/engine.md` FIRST.** It describes the engine you are actually authoring for: `course.project.json` v4 is the editable project and compiles the active hole to the physics-facing `course.json` v3 manifest. Where any reference below mentions a `.golfcourse` archive, `stage_commands`, or `run_course_analyzers`, that is a PRIOR engine — keep the design intent and map it onto the current proposal-card workflow.
...
7. When operating through an agent, use the in-app builder described in `references/engine.md`: propose typed object mutations, preview them, review current/tee/landing/approach/overview captures, and apply only selected cards. The runtime also retains read/validate MCP tools for precision checks. Use `golf-environment-vibe` for dressing and `golf-biome-transitions` for coastline/ecotone work.
...
- Make each changed semantic object its own undoable history event. Keep dependency links explicit so the builder can include prerequisites and explain blocked or cascading reversions.
